...
var ZupraDashboard = {
	...
	_setup: [
		...
		{ label: "Payment Terms", route: "/app/payment-terms-template", done: false },
		{
			...
			route: "/app/sales-taxes-and-charges-template",
			done: false,
		},
	...
	render: function () {
		...
		var pending = this._setup.filter(function (i) {
			return !i.done;
		});
		...
		return [
			...
			'    <p class="zd-sub">Your Zupra Tech ERP is ready. ' +
				done_cnt +
				" of " +
				this._setup.length +
				" setup steps complete.</p>",
			"  </div>",
			...
			"        Attention Required",
			pending.length ? '<span class="zd-badge">' + pending.length + "</span>" : "",
			"      </h2>",
			"    </div>",
			pending.length
				? [
						'<div class="zd-pb-track"><div class="zd-pb-fill" style="width:' +
							pct +
							'%"></div></div>',
						'<p class="zd-pb-lbl">' +
							done_cnt +
						...
						pending
							...
							.map(function (it) {
								return (
									...
									'<span class="zd-cl-lbl">' +
									frappe.utils.escape_html(it.label) +
									"</span>" +
									'<a href="' +
									it.route +
									'" class="zd-btn-cfg">Configure &rarr;</a>' +
									"</div>"
								);
							})
							.join(""),
						pending.length > 6
							? '<p class="zd-see-all"><a href="#" class="zd-link" data-action="show-all-setup">+ ' +
								(pending.length - 6) +
								" more items</a></p>"
							: "",
					].join("")
			...
			this._quicklinks
				.map(function (q) {
					...
					return (
						...
						q.route +
						'" class="zd-ql">' +
						'<div class="zd-ql-ico" style="background:' +
						q.color +
						"1a;color:" +
						q.color +
						'">' +
	...
	load_recent: function (wrapper) {
		frappe.call({
			...
			args: {
				doctype: "View Log",
				fields: ["reference_doctype", "reference_name as reference_document", "creation"],
			...
			callback: function (r) {
				...
				var rows = r && r.message && r.message.length ? r.message : [];
				...
				el.innerHTML = rows
					.map(function (row) {
						var route =
							"/app/" +
							frappe.router.slug(row.reference_doctype) +
							"/" +
						...
						return (
							...
							route +
							'" class="zd-recent-row">' +
							'<span class="zd-recent-type">' +
							frappe.utils.escape_html(row.reference_doctype) +
							"</span>" +
							'<span class="zd-recent-name">' +
							frappe.utils.escape_html(row.reference_document) +
							"</span>" +
							"</a>"
						);
					})
